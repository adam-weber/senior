...
/*******************************************************************************
* Function Name: FRamInterface_I2C_ISR
****************************************************************************//**
*
*  Handles the Interrupt Service Routine for the SCB I2C mode.
*
*******************************************************************************/
CY_ISR(FRamInterface_I2C_ISR)
{
    uint32 diffCount;
    uint32 endTransfer;

#ifdef FRamInterface_I2C_ISR_ENTRY_CALLBACK
    FRamInterface_I2C_ISR_EntryCallback();
#endif /* FRamInterface_I2C_ISR_ENTRY_CALLBACK */

#if (FRamInterface_I2C_CUSTOM_ADDRESS_HANDLER_CONST)
    uint32 response;

    response = FRamInterface_I2C_ACK_ADDR;
    ...
    if(NULL != FRamInterface_customIntrHandler)
    {
        FRamInterface_customIntrHandler();
    }

    if(FRamInterface_CHECK_INTR_I2C_EC_MASKED(FRamInterface_INTR_I2C_EC_WAKE_UP))
    {
    ...
    */
    if(FRamInterface_CHECK_I2C_FSM_MASTER)
    {
        #if(FRamInterface_I2C_MASTER)
        {
            ...
            */
            if(FRamInterface_CHECK_INTR_MASTER_MASKED(FRamInterface_INTR_MASTER_I2C_BUS_ERROR))
            {
                FRamInterface_mstrStatus |= (uint16) (FRamInterface_I2C_MSTAT_ERR_XFER |
                                                         FRamInterface_I2C_MSTAT_ERR_BUS_ERROR);

                endTransfer = FRamInterface_I2C_CMPLT_ANY_TRANSFER;
            }
            ...
            */
            if(FRamInterface_CHECK_INTR_MASTER_MASKED(FRamInterface_INTR_MASTER_I2C_ARB_LOST))
            {
                FRamInterface_mstrStatus |= (uint16) (FRamInterface_I2C_MSTAT_ERR_XFER |
                                                         FRamInterface_I2C_MSTAT_ERR_ARB_LOST);

                endTransfer = FRamInterface_I2C_CMPLT_ANY_TRANSFER;
            }

            #if(FRamInterface_I2C_MULTI_MASTER_SLAVE)
            {
                /* I2C_MASTER_CMD_M_START_ON_IDLE:
                * MultiMaster-Slave does not generate start, because Slave was addressed.
                * Pass control to slave.
                */
                if(FRamInterface_CHECK_I2C_MASTER_CMD(FRamInterface_I2C_MASTER_CMD_M_START_ON_IDLE))
                {
                    FRamInterface_mstrStatus |= (uint16) (FRamInterface_I2C_MSTAT_ERR_XFER |
                                                             FRamInterface_I2C_MSTAT_ERR_ABORT_XFER);
            ...
            */
            if(0u != endTransfer)
            {
                ...
                FRamInterface_mstrStatus |= (uint16) FRamInterface_GET_I2C_MSTAT_CMPLT;
                ...
                #if(FRamInterface_I2C_MULTI_MASTER_SLAVE)
                {
                    if(FRamInterface_CHECK_I2C_FSM_ADDR)
                    {
                        ...
                        */
                        if(FRamInterface_CHECK_I2C_MASTER_CMD(FRamInterface_I2C_MASTER_CMD_M_START_ON_IDLE))
                        {
                            FRamInterface_I2C_MASTER_CLEAR_START;

                            endTransfer = FRamInterface_I2C_CMPLT_ANY_TRANSFER; /* Pass control to Slave */
                        }
                        ...
                        */
                        else if((!FRamInterface_CHECK_INTR_SLAVE_MASKED(FRamInterface_INTR_SLAVE_I2C_BUS_ERROR))
                               && FRamInterface_CHECK_INTR_MASTER_MASKED(FRamInterface_INTR_MASTER_I2C_ARB_LOST))
                        {
                            endTransfer = FRamInterface_I2C_CMPLT_ANY_TRANSFER; /* Pass control to Slave */
                        }
                        else
                        ...
                        {
                            FRamInterface_CLEAR_TX_FIFO; /* Shifter keeps address, clear it */

                            FRamInterface_DISABLE_MASTER_AUTO_DATA_ACK; /* In case of reading disable autoACK */
                        ...
                        else
                        {
                            ...
                            FRamInterface_state = FRamInterface_I2C_FSM_EXIT_IDLE;
                        }
                    }
                    else
                    {
    ...
    else /* (FRamInterface_CHECK_I2C_FSM_SLAVE) */
    {
        #if(FRamInterface_I2C_SLAVE)
        {
            /* INTR_SLAVE_I2C_BUS_ERROR or FRamInterface_INTR_SLAVE_I2C_ARB_LOST:
            * A Misplaced Start or Stop condition occurred on the bus: set a flag
            * to notify an error condition.
            */
            if(FRamInterface_CHECK_INTR_SLAVE_MASKED(FRamInterface_INTR_SLAVE_I2C_BUS_ERROR |
                                                        FRamInterface_INTR_SLAVE_I2C_ARB_LOST))
            {
                if(FRamInterface_CHECK_I2C_FSM_RD)
                {
                    /* TX direction: master reads from slave */
                    FRamInterface_slStatus &= (uint8) ~FRamInterface_I2C_SSTAT_RD_BUSY;
                    FRamInterface_slStatus |= (uint8) (FRamInterface_I2C_SSTAT_RD_ERR |
                                                          FRamInterface_I2C_SSTAT_RD_CMPLT);
                }
                else
                {
                    /* RX direction: master writes into slave */
                    FRamInterface_slStatus &= (uint8) ~FRamInterface_I2C_SSTAT_WR_BUSY;
                    FRamInterface_slStatus |= (uint8) (FRamInterface_I2C_SSTAT_WR_ERR |
                                                          FRamInterface_I2C_SSTAT_WR_CMPLT);
                }
    ...
    */
    ...
    if(FRamInterface_CHECK_I2C_FSM_MASTER)
    {
        #if(FRamInterface_I2C_MASTER)
        {
            ...
            */
            if(FRamInterface_CHECK_INTR_MASTER_MASKED(FRamInterface_INTR_MASTER_I2C_STOP))
            {
                FRamInterface_ClearMasterInterruptSource(FRamInterface_INTR_MASTER_I2C_STOP);
            ...
            {
                if(FRamInterface_CHECK_I2C_FSM_ADDR) /* Address stage */
                {
                    /* INTR_MASTER_I2C_NACK:
                    * The master sent an address but it was NACKed by the slave. Complete transaction.
                    */
                    if(FRamInterface_CHECK_INTR_MASTER_MASKED(FRamInterface_INTR_MASTER_I2C_NACK))
                    {
                        FRamInterface_ClearMasterInterruptSource(FRamInterface_INTR_MASTER_I2C_NACK);

                        FRamInterface_mstrStatus |= (uint16) (FRamInterface_I2C_MSTAT_ERR_XFER |
                                                                 FRamInterface_I2C_MSTAT_ERR_ADDR_NAK);
                    ...
                    */
                    ...
                    {
                        if(FRamInterface_CHECK_I2C_FSM_RD) /* Reading */
                        {
                        ...
                        else /* Writing */
                        {
                            ...
                            if(0u != FRamInterface_mstrWrBufSize)
                            {
                                /* Enable INTR.TX_EMPTY if there is data to transmit */
                                FRamInterface_SetTxInterruptMode(FRamInterface_INTR_TX_EMPTY);
                            }
                        }
                    }
                }

                if(FRamInterface_CHECK_I2C_FSM_DATA) /* Data phase */
                {
                    if(FRamInterface_CHECK_I2C_FSM_RD) /* Reading */
                    {
                        /* INTR_RX_FULL:
                        ...
                        */
                        if(FRamInterface_CHECK_INTR_RX_MASKED(FRamInterface_INTR_RX_FULL))
                        {
                            /* Calculate difference */
                            diffCount =  FRamInterface_mstrRdBufSize -
                                        (FRamInterface_mstrRdBufIndex + FRamInterface_GET_RX_FIFO_ENTRIES);

                            /* Proceed transaction or end it when RX FIFO becomes FULL again */
                            if(diffCount > FRamInterface_I2C_FIFO_SIZE)
                            {
                                diffCount = FRamInterface_I2C_FIFO_SIZE;
                            }
                            else
                            {
                                if(0u == diffCount)
                                {
                                    FRamInterface_DISABLE_MASTER_AUTO_DATA_ACK;
                            ...
                            {
                                FRamInterface_mstrRdBufPtr[FRamInterface_mstrRdBufIndex] = (uint8)
                                                                                        FRamInterface_RX_FIFO_RD_REG;
                                FRamInterface_mstrRdBufIndex++;
                            }
                        }
                        /* INTR_RX_NOT_EMPTY:
                        ...
                        */
                        else if(FRamInterface_CHECK_INTR_RX_MASKED(FRamInterface_INTR_RX_NOT_EMPTY))
                        {
                            /* Put data in component buffer */
                            FRamInterface_mstrRdBufPtr[FRamInterface_mstrRdBufIndex] = (uint8) FRamInterface_RX_FIFO_RD_REG;
                            FRamInterface_mstrRdBufIndex++;

                            if(FRamInterface_mstrRdBufIndex < FRamInterface_mstrRdBufSize)
                            {
                    ...
                    else /* Writing */
                    {
                        ...
                        */
                        if(FRamInterface_CHECK_INTR_MASTER_MASKED(FRamInterface_INTR_MASTER_I2C_NACK))
                        {
                            ...
                            FRamInterface_mstrWrBufIndexTmp -= (FRamInterface_GET_TX_FIFO_ENTRIES +
                                                                   FRamInterface_GET_TX_FIFO_SR_VALID);

                            /* Update number of transferred bytes */
                            FRamInterface_mstrWrBufIndex = FRamInterface_mstrWrBufIndexTmp;

                            FRamInterface_mstrStatus |= (uint16) (FRamInterface_I2C_MSTAT_ERR_XFER |
                                                                     FRamInterface_I2C_MSTAT_ERR_SHORT_XFER);

                            FRamInterface_CLEAR_TX_FIFO;
                        ...
                        */
                        else if(FRamInterface_CHECK_INTR_TX_MASKED(FRamInterface_INTR_TX_EMPTY))
                        {
                            while(FRamInterface_I2C_FIFO_SIZE != FRamInterface_GET_TX_FIFO_ENTRIES)
                            {
                                ...
                                */
                                if(FRamInterface_mstrWrBufIndexTmp < FRamInterface_mstrWrBufSize)
                                {
                                    ...
                                    */
                                    if(FRamInterface_mstrWrBufIndexTmp == (FRamInterface_mstrWrBufSize - 1u))
                                    {
                                    ...
                                    FRamInterface_TX_FIFO_WR_REG = (uint32) FRamInterface_mstrWrBufPtr[FRamInterface_mstrWrBufIndexTmp];
                                    FRamInterface_mstrWrBufIndexTmp++;
                                }
                                else
                                {
                        ...
                        #else
                            FRamInterface_ClearTxInterruptSource(FRamInterface_INTR_TX_EMPTY);
                        #endif /* (FRamInterface_CY_SCBIP_V0) */
                        }
                        /* INTR_TX_UNDERFLOW:
                        ...
                        */
                        else if(FRamInterface_CHECK_INTR_TX_MASKED(FRamInterface_INTR_TX_UNDERFLOW))
                        {
                            /* Update number of transferred bytes */
                            FRamInterface_mstrWrBufIndex = FRamInterface_mstrWrBufIndexTmp;
                ...
                if(0u != endTransfer) /* Complete transfer */
                {
                    ...
                    if(FRamInterface_CHECK_I2C_MODE_NO_STOP(FRamInterface_mstrControl))
                    {
                        ...
                        FRamInterface_mstrStatus |= (uint16) (FRamInterface_I2C_MSTAT_XFER_HALT |
                                                                 FRamInterface_GET_I2C_MSTAT_CMPLT);

                        FRamInterface_state = FRamInterface_I2C_FSM_MSTR_HALT;
                    }
                    else
                    {
        ...
        } /* (FRamInterface_I2C_MASTER) */
        #endif

    } /* (FRamInterface_CHECK_I2C_FSM_MASTER) */


    /* FSM Slave */
    else if(FRamInterface_CHECK_I2C_FSM_SLAVE)
    {
        #if(FRamInterface_I2C_SLAVE)
        {
            /* INTR_SLAVE_NACK:
            ...
            */
            if(FRamInterface_CHECK_INTR_SLAVE_MASKED(FRamInterface_INTR_SLAVE_I2C_NACK))
            {
                FRamInterface_ClearSlaveInterruptSource(FRamInterface_INTR_SLAVE_I2C_NACK);

                /* All entries that remain in TX FIFO are: FIFO Size + 1 (SHIFTER) */
                diffCount = (FRamInterface_GET_TX_FIFO_ENTRIES + FRamInterface_GET_TX_FIFO_SR_VALID);

                if(FRamInterface_slOverFlowCount > diffCount) /* Overflow */
                {
                    FRamInterface_slStatus |= (uint8) FRamInterface_I2C_SSTAT_RD_OVFL;
                }
                else /* No Overflow */
                {
                    /* Roll-back temporary index */
                    FRamInterface_slRdBufIndexTmp -= (diffCount - FRamInterface_slOverFlowCount);
                }

                /* Update slave of transferred bytes */
                FRamInterface_slRdBufIndex = FRamInterface_slRdBufIndexTmp;
                ...
                FRamInterface_slStatus &= (uint8) ~FRamInterface_I2C_SSTAT_RD_BUSY;
                FRamInterface_slStatus |= (uint8)  FRamInterface_I2C_SSTAT_RD_CMPLT;
                FRamInterface_state     =  FRamInterface_I2C_FSM_IDLE;
                
            #ifdef FRamInterface_I2C_SLAVE_CMPLT_CALLBACK
                /* Read complete */
                FRamInterface_I2C_SlaveCompleteCallback();
            #endif /* FRamInterface_I2C_SLAVE_CMPLT_CALLBACK */
            }
            ...
            */
            if(FRamInterface_CHECK_INTR_SLAVE_MASKED(FRamInterface_INTR_SLAVE_I2C_WRITE_STOP))
            {
                FRamInterface_ClearSlaveInterruptSource(FRamInterface_INTR_SLAVE_I2C_WRITE_STOP);
                ...
                * I2C_ADDR_MATCH.
                */
                if (0u != (FRamInterface_I2C_CTRL_REG & FRamInterface_I2C_CTRL_S_READY_DATA_ACK))
                {
                    while(0u != FRamInterface_GET_RX_FIFO_ENTRIES)
                    {
                        #if(FRamInterface_CHECK_I2C_ACCEPT_ADDRESS)
                        {
                            if((1u == FRamInterface_GET_RX_FIFO_ENTRIES) &&
                               (FRamInterface_CHECK_INTR_SLAVE_MASKED(FRamInterface_INTR_SLAVE_I2C_ADDR_MATCH)))
                            {
                        ...
                        /* Put data in component buffer */
                        FRamInterface_slWrBufPtr[FRamInterface_slWrBufIndex] = (uint8) FRamInterface_RX_FIFO_RD_REG;
                        FRamInterface_slWrBufIndex++;
                    }

                    FRamInterface_DISABLE_SLAVE_AUTO_DATA;
                }

                if(FRamInterface_CHECK_INTR_RX(FRamInterface_INTR_RX_OVERFLOW))
                {
                    FRamInterface_slStatus |= (uint8) FRamInterface_I2C_SSTAT_WR_OVFL;
                }
                ...
                FRamInterface_slStatus &= (uint8) ~FRamInterface_I2C_SSTAT_WR_BUSY;
                FRamInterface_slStatus |= (uint8)  FRamInterface_I2C_SSTAT_WR_CMPLT;
                FRamInterface_state     =  FRamInterface_I2C_FSM_IDLE;
            ...
            */
        ...
        #endif /* (FRamInterface_I2C_CUSTOM_ADDRESS_HANDLER_CONST) */
            {
                ...
                FRamInterface_ClearI2CExtClkInterruptSource(FRamInterface_INTR_I2C_EC_WAKE_UP);

                #if (FRamInterface_I2C_CUSTOM_ADDRESS_HANDLER)
                {
                    if (NULL != FRamInterface_customAddressHandler)
                    {
                        /* Call custom address handler */
                        response = FRamInterface_customAddressHandler();
                    }
                    else
                    {
                        /* Read address from the RX FIFO. If there is no address underflow triggers but
                        * component does not use that source. */
                        (void) FRamInterface_RX_FIFO_RD_REG;
                        response = FRamInterface_I2C_ACK_ADDR;
                    }
            ...
            #if (FRamInterface_I2C_CUSTOM_ADDRESS_HANDLER_CONST)
                if (response == FRamInterface_I2C_NAK_ADDR)
                {
                #if (!FRamInterface_CY_SCBIP_V0)
                    /* Disable write stop interrupt source as it triggers after address was NACKed. Ticket ID#156094 */
                    FRamInterface_DISABLE_INTR_SLAVE(FRamInterface_INTR_SLAVE_I2C_WRITE_STOP);
                #endif /* (!FRamInterface_CY_SCBIP_V0) */
                    ...
                    FRamInterface_I2C_SLAVE_GENERATE_NACK;
                }
                else
            #endif /* (FRamInterface_I2C_CUSTOM_ADDRESS_HANDLER_CONST) */
                {
                    ...
                    /* TX direction: master reads from slave */
                    {
                        ...
                        FRamInterface_slRdBufIndexTmp = FRamInterface_slRdBufIndex;

                        /* Start master reading */
                        FRamInterface_slStatus |= (uint8) FRamInterface_I2C_SSTAT_RD_BUSY;
                        FRamInterface_state     = FRamInterface_I2C_FSM_SL_RD;
                    }
                    else
                    /* RX direction: master writes into slave */
                    {
                        /* Calculate available buffer size */
                        diffCount = (FRamInterface_slWrBufSize - FRamInterface_slWrBufIndex);
                        ...
                        /* Receive data: into RX FIFO */
                        {
                            if(diffCount == FRamInterface_I2C_FIFO_SIZE)
                            {
                                ...
                                FRamInterface_ENABLE_SLAVE_AUTO_DATA;
                            }
                            else
                            {
                                /* Stretch clock when RX FIFO becomes FULL */
                                FRamInterface_ENABLE_SLAVE_AUTO_DATA_ACK;
                        ...
                        /* Start master reading */
                        FRamInterface_slStatus |= (uint8) FRamInterface_I2C_SSTAT_WR_BUSY;
                        FRamInterface_state     = FRamInterface_I2C_FSM_SL_WR;
                    }
                    ...
                    FRamInterface_ENABLE_INTR_SLAVE(FRamInterface_INTR_SLAVE_I2C_WRITE_STOP);
                #endif /* (!FRamInterface_CY_SCBIP_V0) */

                    /* ACK the address byte */
                    FRamInterface_I2C_SLAVE_GENERATE_ACK;
                }
            }
            ...
            */
            if(FRamInterface_CHECK_INTR_RX_MASKED(FRamInterface_INTR_RX_FULL))
            {
                ...
                diffCount =  FRamInterface_slWrBufSize -
                            (FRamInterface_slWrBufIndex + FRamInterface_I2C_FIFO_SIZE);

                if(diffCount > FRamInterface_I2C_FIFO_SIZE) /* Proceed transaction */
                {
                    diffCount   = FRamInterface_I2C_FIFO_SIZE;
                    endTransfer = 0u;  /* Continue active transfer */
                }
                ...
                {
                    FRamInterface_ENABLE_SLAVE_AUTO_DATA_NACK;
                ...
                FRamInterface_ClearRxInterruptSource(FRamInterface_INTR_RX_FULL);
            }
            ...
            */
            ...
            {
                diffCount = FRamInterface_RX_FIFO_RD_REG;

                if(FRamInterface_slWrBufIndex < FRamInterface_slWrBufSize)
                {
                    FRamInterface_I2C_SLAVE_GENERATE_ACK;

                    /* Put data into component buffer */
                    FRamInterface_slWrBufPtr[FRamInterface_slWrBufIndex] = (uint8) diffCount;
                    FRamInterface_slWrBufIndex++;
                }
                ...
                FRamInterface_ClearRxInterruptSource(FRamInterface_INTR_RX_NOT_EMPTY);
            }
            else
            {
                /* Does nothing */
            ...
            */
            if(FRamInterface_CHECK_INTR_TX_MASKED(FRamInterface_INTR_TX_EMPTY))
            {
                while(FRamInterface_I2C_FIFO_SIZE != FRamInterface_GET_TX_FIFO_ENTRIES)
                {
                    ...
                    */
                    if(FRamInterface_slRdBufIndexTmp < FRamInterface_slRdBufSize)
                    /* Data from buffer */
                    {
                        FRamInterface_TX_FIFO_WR_REG = (uint32) FRamInterface_slRdBufPtr[FRamInterface_slRdBufIndexTmp];
                        FRamInterface_slRdBufIndexTmp++;
                    }
                    else
                    /* Probably Overflow */
                    {
                        FRamInterface_TX_FIFO_WR_REG = FRamInterface_I2C_SLAVE_OVFL_RETURN;

                        if(FRamInterface_slOverFlowCount <= FRamInterface_I2C_TX_OVERFLOW_COUNT)
                        {
                            /* Get counter in range of overflow. */
                            FRamInterface_slOverFlowCount++;
                        }
                    }
                }
...
#ifdef FRamInterface_I2C_ISR_EXIT_CALLBACK
    FRamInterface_I2C_ISR_ExitCallback();
#endif /* FRamInterface_I2C_ISR_EXIT_CALLBACK */
