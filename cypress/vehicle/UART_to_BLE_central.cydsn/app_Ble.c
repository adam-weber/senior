...
//            count = 0;
            ...
            else if(CyBle_GattGetBusStatus() != CYBLE_STACK_STATE_BUSY && send ==true)
            {
                
                extern uint8 flash_storage;
                //if ((int)flash_storage < 0){
                 //   count = 1;
                //StoreInFlash((uint8*)count, 16);
               // }
                //count = (int)flash_storage;
                //StoreInFlash((uint8*)count, 16);
                HandleUartTxTraffic();
